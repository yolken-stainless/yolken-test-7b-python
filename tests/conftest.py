...
from yolken_test_7b import YolkenTest7b, AsyncYolkenTest7b, DefaultAioHttpClient
from yolken_test_7b._utils import is_dict
...
logging.getLogger("yolken_test_7b").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[YolkenTest7b]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with YolkenTest7b(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncYolkenTest7b]:
